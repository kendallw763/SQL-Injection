...
//this function is responsable for running the query to access the database using regex to simulate common sql injection code
std::string input;//declare the input variable 
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  std::regex sqlInjectionPattern("('|--|;|\\/\\*|\\*\\/|\\bor\\b|\\band\\b|\\bdrop\\b|\\bdelete\\b|\\binsert\\b|\\bselect\\b|\\bunion\\b)", std::regex::icase);
  //this used the regex search to check if the input matches the pattern. In other words, if so, there is a breach
    return std::regex_search(input, sqlInjectionPattern); 
 
} 
 

  // declerations from external files ned to implemented. Otherwise, the console will have no output.
  extern sqlite3*db;//this serves as the database connection
  extern std::string sql; // this is the query sting
  extern int callback(void*, char**, char**, int);//this serves as the callback function
  extern void* records; //this is the records data structure


bool queryDatabse(){
  char* error_message = nullptr;
//if the sql code is not okay, there will be an error message and return false to stop the program. 
  if(sqlite3_exec(db, sql.c_str(), callback, &records, &error_message) != SQLITE_OK){
    ...
    sqlite3_free(error_message);//generate error message
    return false;
} 
//Otherwise, we can continue to run the program normally
  return true;
}
